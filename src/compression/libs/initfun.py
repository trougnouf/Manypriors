...
from compression.libs import json_saver
...
def common_parser_add_arguments(parser) -> None:
    ...
    parser.add_argument('--save_path', help='Save path, this argument is normally autogenerated as checkpoints/expname')
...
def get_args_jsonsaver(specific_parser_add_arguments_fun: Callable, specific_parser_autocomplete_fun: Callable, cli_args: Optional[List[str]] = None):
    parser = configargparse.ArgumentParser(description=__doc__, default_config_files=['configs/defaults.yaml'], config_file_parser_class=configargparse.YAMLConfigFileParser)
...
def common_init_parser_autocomplete(args: configargparse.Namespace) -> None:
    ...
    if args.config is None and args.pretrain is not None:
        ...
        args.config = os.path.join('checkpoints', args.pretrain, 'config.yaml')
        logger.info('common_init_parser_autocomplete: missing config. Did you mean to run with "--config {}"?'.format(os.path.join('checkpoints', args.pretrain, 'config.yaml')))
    ...
    if args.save_path is None:
        args.save_path = os.path.join('checkpoints', args.expname)
...
def get_best_checkpoint(exp, checkpoints_dir='checkpoints', prefix='test', step=None, suffix='combined_loss',
    ...
    if metric not in results['best_step'] and prefix == 'test':
        metric = "{}_{}".format('test', suffix)
